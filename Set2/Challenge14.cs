...
namespace cryptoPalsSet2
{
    ...
    {
        static string challenge14()
        {
            //Plaintext versions of values to be encrypted
            string secretKeyPlain = "BBBBBBBBBBBBBBBB";
            string knownText = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
            ...
            //Appending a random amount of random bytes (this is the only difference to 12)
            Random rnd = new Random();
            Byte[] randomBytes = new Byte[rnd.Next(1,100)];
            ...
            int blockSize = IdentifyBlockSize14(keyBytes);


            string final = decryptor14(randomBytes, knownBytes, cipherBytes, keyBytes, blockSize);
            Console.WriteLine(final);
            return "";
        }

        public static string decryptor14(byte[] randomBytes, byte[] knownBytes,
            byte[] encryptedBytes, byte[] key, int blockSize)
        {
            byte[] data = new byte[randomBytes.Length + knownBytes.Length + encryptedBytes.Length];

            Array.Copy(randomBytes, 0, data, 0, randomBytes.Length);
            Array.Copy(knownBytes, 0, data, randomBytes.Length, knownBytes.Length);
            ...
            for (int a = blockSize - 1; a < data.Length - blockSize; a++)
            {
                ...
                dict = populateDict14(block, key, blockSize);
        ...
        public static Dictionary<int, byte[]> populateDict14(byte[] data,
        ...
        public static int IdentifyBlockSize14(byte[] key)
        {
            int keySize = 0;
            string plainText = "";
